...
describe('use case update location', () => {
  it('should update location successfully', async () => {
    await User.updateUserLocation('Jack', 37.4106, -122.059732);
  ...
  it('should retrieve user locations successfully', async () => {
    await User.updateUserLocation('Jack', 37.4106, -122.059732);
    const actual = await User.retrieveUserLocations();
    const expected = [
      {
        username: 'John',
  ...
  it('should get one user location successfully', async () => {
    const actual = await User.retrieveUserLocation('John');
    const expected = {
      location: { longitude: 1, latitude: 2 },
    };
  ...
  it('should delete location successfully', async () => {
    await User.deleteUserLocations('John');
    const result = await User.findOne(
      { username: 'John' },
      { _id: 0, location: 1 }
    );
